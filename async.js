...
//                 // console.log(response.data) //(url + idx);
//                 // count++
//             })
...
db.stock.findAll({ where: { userId: 1 } })
    ...
    async.parallel(something, function (err, results) {
     ...
     Object.keys(results).forEach(k => { 
        ...
        datasets = Object.keys(obj).map(k => {
            return {
                label: k,
                data: obj[k],
                backgroundColor: [
...
// return (results[k].data['Meta Data']['2. Symbol'])[
//     (results[k].data['Time Series (Daily)'][today]['4. close']),
//     (results[k].data['Time Series (Daily)'][t7]['4. close']),
//     (results[k].data['Time Series (Daily)'][t14]['4. close']),
//     (results[k].data['Time Series (Daily)'][t21]['4. close']),
//     (results[k].data['Time Series (Daily)'][t28]['4. close'])
// ]